...
import type { Employee, FocusGroup } from "@/types"
import { useEffect, useState } from "react"
import { Link } from "react-router"

// Import skeleton components
import FocusGroupHeaderSkeleton from "../components/focus-group/Focus-group-headerSkeleton"
import ParticipantsTableSkeleton from "../components/focus-group/Participating-tableSkeleton"
import { ActionPlansCarouselContent } from "../components/ActionPlan/Action-plan-carousel-content"
...
export default function FocusGroupDetails({
  ...
  const [isActionPlansLoading, setIsActionPlansLoading] = useState(true)
  ...
  useEffect(() => {
    ...
    // Simulate header data loading faster than participants
    setTimeout(() => {
      setIsHeaderLoading(false)
    }, 800)

    // Simulate action plans loading
    setTimeout(() => {
      setIsActionPlansLoading(false)
    }, 1200)

    fetch(`${import.meta.env.VITE_BACKEND_URL}/api/groups/${selectedGroup.focus_group_id}`)
      .then((res) => res.json())
      .then((data) => {
        console.log(data)
  ...
  const handleApplyFilters = (filters: {
    ...
    const filtered = participants.filter((participant) => {
      ...
      const jobTitleMatches = filters.jobTitles.includes("All") || filters.jobTitles.includes(participant.job_title)

      // Check if date added matches filter
      const dateAddedMatches = filters.dateAdded.includes("All") || filters.dateAdded.includes(participant.joining_date)
  ...
  const removeSelectedParticipants = () => {
    ...
    const updatedParticipants = participants.filter((_, index) => !selectedParticipants.includes(index))
  ...
  const handleAddNewParticipants = (selectedIds: string[]) => {
    ...
    const newParticipants = allEmployees.filter((emp) => selectedIds.includes(emp.employee_id))
  ...
  const participantMatchesFilters = (participant: Employee) => {
    ...
    const jobTitleMatches =
      participantFilters.jobTitles.includes("All") || participantFilters.jobTitles.includes(participant.job_title)

    // Check if date added matches filter
    const dateAddedMatches =
      participantFilters.dateAdded.includes("All") || participantFilters.dateAdded.includes(participant.joining_date)
  ...
  return (
    <div className="flex-1 overflow-auto">
      ...
      <main className="p-6 pt-0">
        ...
        {isHeaderLoading ? (
          <FocusGroupHeaderSkeleton />
        ) : (
          <div className="rounded-lg border border-gray-200 bg-white p-6 mb-6">
            <div className="flex justify-between items-center">
              <div>
                <h1 className="text-2xl font-bold text-gray-900">{selectedGroup.name}</h1>
                <p className="text-sm text-gray-500 mt-2">Group ID: {selectedGroup.focus_group_id}</p>
              </div>
        ...
        <div className="mb-6">
          <ActionPlansCarouselContent isLoading={isActionPlansLoading}>
            <ActionPlansCarousel />
          </ActionPlansCarouselContent>
        </div>
        ...
        <div className="mt-8">
          ...
          <div className="mb-4 flex justify-between items-center">
            ...
            <div className="flex items-center space-x-3">
              <FilterButton onClick={handleFilter} isActive={hasActiveFilters()} />
              <button
          ...
          {isParticipantsLoading ? (
            <ParticipantsTableSkeleton rowCount={5} />
          ) : (
