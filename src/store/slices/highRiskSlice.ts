...
const highRiskEmployeeSlice = createSlice({
  ...
  reducers: {
    ...
    setEscalation: (
      state,
      action: PayloadAction<{ employee_id: string; escalation: boolean }>
    ) => {
      const { employee_id, escalation } = action.payload;
      const employee = state.highRiskEmployees.find(
    ...
    setMeetScheduled: (
      state,
      action: PayloadAction<{ employee_id: string; meet_scheduled: boolean }>
    ) => {
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      fetchHighRiskEmployees.fulfilled,
    ...
    builder.addCase(fetchHighRiskEmployees.rejected, (state, action) => {
      state.loading = false;
      console.error(
        "Failed to fetch high-risk employees:",
...
export const { setEmployees, setEscalation, setMeetScheduled } =
  highRiskEmployeeSlice.actions;
