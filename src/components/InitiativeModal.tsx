...
const InitiativeModal: FC<InitiativeModalProps> = ({ onClose }) => {
  const [activeTab, setActiveTab] = useState<'basicInfo' | 'targetGroups' | 'actionSteps'>('basicInfo');
  const [indicatorStyle, setIndicatorStyle] = useState({ left: '0%', width: '33.333%' });
  ...
  const metrics = ['Morale', 'Engagement', 'Cultural Score', 'Leave Impact', 'Team Collaboration', 'Innovation', 'Customer Satisfaction', 'Productivity', 'Leadership', 'Professional Growth', 'Work-Life Balance', 'Retention Rate'];
  ...
  const addTargetGroup = () => {
    if (targetGroupInput.trim() !== '' && !targetGroups.includes(targetGroupInput.trim())) {
      ...
      setActionSteps(prev => 
        prev.map(step => 
          step.id === editingStepId 
            ? { ...step, title: stepTitle.trim(), description: stepDescription.trim() } 
            : step
        )
      );
      setStepTitle('');
      ...
      <div className="w-full max-w-5xl rounded-lg bg-white shadow-xl">
        ...
        <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
          <div className="flex items-center">
            ...
            <div>
              <h2 className="text-2xl font-semibold text-gray-900">Create Initiative</h2>
          ...
          >
            <svg className="mr-2 h-5 w-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M18 21C19.6569 21 21 19.6569 21 18C21 16.3431 19.6569 15 18 15C16.3431 15 15 16.3431 15 18C15 19.6569 16.3431 21 18 21Z" 
                stroke={activeTab === 'targetGroups' ? '#80C342' : completedTabs.includes('basicInfo') ? '#6b7280' : '#d1d5db'} 
                strokeWidth="2" 
                strokeLinecap="round" 
                strokeLinejoin="round"
              />
          ...
          >
            <svg className="mr-2 h-5 w-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M9 5H7C5.89543 5 5 5.89543 5 7V19C5 20.1046 5.89543 21 7 21H17C18.1046 21 19 20.1046 19 19V7C19 5.89543 18.1046 5 17 5H15M9 5C9 6.10457 9.89543 7 11 7H13C14.1046 7 15 6.10457 15 5M9 5C9 3.89543 9.89543 3 11 3H13C14.1046 3 15 3.89543 15 5M12 12H15M12 16H15M9 12H9.01M9 16H9.01" 
                stroke={activeTab === 'actionSteps' ? '#80C342' : completedTabs.includes('targetGroups') ? '#6b7280' : '#d1d5db'} 
                strokeWidth="2" 
                strokeLinecap="round" 
                strokeLinejoin="round"
              />
        ...
        <div className="p-6 min-h-[320px] overflow-y-auto max-h-[60vh]">
          <div className={`transition-opacity duration-300 ${activeTab === 'basicInfo' ? 'opacity-100' : 'opacity-0 hidden'}`}>
            <div className="mb-6">
              <div className="mb-1 flex items-baseline">
                <span className="mr-2 inline-flex h-6 w-6 items-center justify-center rounded-full bg-gray-200 text-sm font-semibold text-gray-700">1</span>
            ...
            <div className="mb-6">
              <div className="mb-1 flex items-baseline">
                <span className="mr-2 inline-flex h-6 w-6 items-center justify-center rounded-full bg-gray-200 text-sm font-semibold text-gray-700">2</span>
          ...
          <div className={`transition-opacity duration-300 ${activeTab === 'targetGroups' ? 'opacity-100' : 'opacity-0 hidden'}`}>
            <div className="mb-6">
              <div className="mb-1 flex items-baseline">
                <span className="mr-2 inline-flex h-6 w-6 items-center justify-center rounded-full bg-gray-200 text-sm font-semibold text-gray-700">3</span>
                <h3 className="text-xl font-semibold text-gray-900">Target Groups</h3>
              </div>
              
              <div className={`mt-3 rounded-lg border ${showTargetGroupsError && targetGroups.length === 0 ? 'border-red-500' : 'border-dashed border-gray-300'} p-4`}>
                {/* Selected Target Groups */}
                {targetGroups.length > 0 && (
                  <div className="mb-3 flex flex-wrap gap-2">
                    {targetGroups.map((group, index) => (
                      <div key={index} className="flex items-center rounded-full bg-[#80C342]/10 px-3 py-1.5 text-sm text-[#80C342] border border-[#80C342]/20">
                        <span>{group}</span>
                        <button 
                ...
                {showTargetGroupsError && targetGroups.length === 0 && (
                  <p className="mt-2 text-sm text-red-500">At least one target group is required</p>
                )}
              </div>
            </div>
            ...
            <div>
              <div className="mb-1 flex items-baseline">
                <span className="mr-2 inline-flex h-6 w-6 items-center justify-center rounded-full bg-gray-200 text-sm font-semibold text-gray-700">4</span>
              ...
              <div className="relative mt-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {selectedMetrics.length > 0 ? (
                      selectedMetrics.map((metric, index) => (
                        <span key={index} className="flex items-center rounded-full bg-[#80C342]/10 px-2 py-0.5 text-xs text-[#80C342] border border-[#80C342]/20">
                ...
                {isMetricDropdownOpen && (
                  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20" onClick={() => setIsMetricDropdownOpen(false)}>
                    <div 
                    ...
                    >
                      ...
                      <div className="mt-4 max-h-60 overflow-y-auto pr-1">
                        {metrics.map((metric, index) => (
                          ...
                          >
                            <div className={`mr-2 h-5 w-5 flex items-center justify-center rounded border ${selectedMetrics.includes(metric) ? 'bg-[#80C342] border-[#80C342]' : 'border-gray-300'}`}>
                ...
                {showTargetGroupsError && selectedMetrics.length === 0 && (
                  <p className="mt-1 text-sm text-red-500">At least one metric is required</p>
                )}
              </div>
            </div>
          </div>

          <div className={`transition-opacity duration-300 ${activeTab === 'actionSteps' ? 'opacity-100' : 'opacity-0 hidden'}`}>
            <div className="mb-6 flex items-baseline">
              <span className="mr-2 inline-flex h-6 w-6 items-center justify-center rounded-full bg-gray-200 text-sm font-semibold text-gray-700">5</span>
                        ...
                        <div className="flex space-x-2">
                          <button
                            ...
                            className={`rounded-md px-4 py-2 text-white transition-colors ${
                              stepTitle.trim() ? 'bg-[#80C342] hover:bg-[#6ba238]' : 'bg-gray-300 cursor-not-allowed'
                            }`}
                          >
                            Update
                          </button>
        ...
        <div className="flex justify-between border-t border-gray-200 p-6">
          ...
          <div className="w-40">
            <div className="relative flex items-center justify-between">
              ...
              {/* Step circles */}
              {['basicInfo', 'targetGroups', 'actionSteps'].map((step, index) => {
                const isActive = activeTab === step;
                const isCompleted = completedTabs.includes(step);
                const nextStepCompleted = index < 2 && (
                  completedTabs.includes(['targetGroups', 'actionSteps'][index]) ||
                  ...
                  <div key={step} className="relative z-10 flex items-center">
                    ...
                    {index < 2 && (
                      <div className="w-11 h-0.5 ml-1">
                        <div className={`h-full ${nextStepCompleted ? 'bg-[#80C342]' : 'bg-gray-200'}`}></div>
                      </div>
                    )}
                  </div>
                );
          ...
          <div className="flex space-x-3">
            {(activeTab === 'targetGroups' || activeTab === 'actionSteps') && (
              ...
              >
                <svg className="mr-2 inline-block h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
            ...
            >
              ...
              {activeTab !== 'actionSteps' && (
                <svg className="ml-2 inline-block h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
