...
const FocusGroupModal: FC<FocusGroupModalProps> = ({ onClose, onSubmit, editingFocusGroup }) => {
  ...
  useEffect(() => {
    if (editingFocusGroup) {
      ...
      if (editingFocusGroup.participantCount > 0) {
        ...
        const mockSelectedEmployees = MOCK_EMPLOYEES.slice(0, editingFocusGroup.participantCount);
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/60 backdrop-blur">
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full">
        <div className="p-4">
          ...
          <div className="mb-4 relative">
            ...
            >
              <div className="flex items-center gap-2 w-full overflow-x-auto whitespace-nowrap hide-scrollbar" style={hideScrollbarStyle}>
                {tags.length > 0 ? (
                  tags.map((tag, index) => (
                    <span 
                      key={index} 
          ...
          <div className="mb-4">
            ...
            {selectedEmployees.length > 0 && (
              <div className="mb-2 border border-gray-200 rounded-lg p-2 overflow-x-auto">
                <div className="flex items-center gap-2 w-full whitespace-nowrap" style={hideScrollbarStyle}>
      ...
      {showEmployeesPopup && (
        <div className="fixed inset-0 z-[60]" onClick={() => setShowEmployeesPopup(false)}>
          <div 
            className="absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg shadow-lg w-full max-w-md"
            onClick={(e) => e.stopPropagation()}
          >
