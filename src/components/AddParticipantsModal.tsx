...
const AddParticipantsModal: FC<AddParticipantsModalProps> = ({
  ...
  const [tempJobTitleFilters, setTempJobTitleFilters] = useState<string[]>(['All']);
  const [tempDateAddedFilters, setTempDateAddedFilters] = useState<string[]>(['All']);
  
  // Get unique job titles and dates from all employees
  const uniqueJobTitles = ['All', ...Array.from(new Set(allEmployees.map(emp => emp.title)))];
  const uniqueDates = ['All', ...Array.from(new Set(allEmployees.map(emp => emp.dateAdded)))].sort();
  ...
  useEffect(() => {
    ...
    const available = allEmployees.filter(emp => !existingIds.includes(emp.id));
  ...
  useEffect(() => {
    let filtered = allEmployees.filter(emp => 
      !existingParticipants.map(p => p.id).includes(emp.id)
    );
    ...
    if (!dateAddedFilters.includes('All')) {
      filtered = filtered.filter(emp => dateAddedFilters.includes(emp.dateAdded));
    }
    
    setFilteredEmployees(filtered);
  }, [searchQuery, allEmployees, existingParticipants, jobTitleFilters, dateAddedFilters]);
  ...
  const hasActiveFilters = () => {
    return !jobTitleFilters.includes('All') || !dateAddedFilters.includes('All');
  };
            ...
            {showFilterOptions && (
              <div className="absolute z-10 mt-1 w-80 bg-white border border-gray-200 rounded-lg shadow-lg right-0 max-h-[450px] flex flex-col">
                ...
                <div className="overflow-y-auto flex-1">
                  ...
                  <div className="p-4 border-b border-gray-200">
                    <h4 className="text-base font-semibold text-gray-900 mb-3">Job Title</h4>
                    <div className="space-y-2 max-h-[150px] overflow-y-auto pr-2">
                      {uniqueJobTitles.map((title, index) => (
                        <div key={index} className="flex items-center">
                          ...
                          />
                          <label htmlFor={`job-title-${index}`} className="ml-2 text-sm text-gray-900">
                            {title}
                          </label>
                        </div>
                      ))}
                  ...
                  <div className="p-4 border-b border-gray-200">
                    <h4 className="text-base font-semibold text-gray-900 mb-3">Date Added</h4>
                    <div className="space-y-2 max-h-[150px] overflow-y-auto pr-2">
                      {uniqueDates.map((date, index) => (
                        <div key={index} className="flex items-center">
                          ...
                          />
                          <label htmlFor={`date-added-${index}`} className="ml-2 text-sm text-gray-900">
                            {date}
                          </label>
                        </div>
                      ))}
                ...
                filteredEmployees.map((employee) => (
                  ...
                  >
                    ...
                    <div className="col-span-2 px-4 flex items-center justify-between">
                      <div className="text-sm text-gray-500">{employee.dateAdded}</div>
