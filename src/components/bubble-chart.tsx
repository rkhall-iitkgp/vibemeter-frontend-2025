...
const BubbleChartPreview = ({ className = "" }) => {
  ...
  useEffect(() => {
    ...
    const data = [
      // Larger bubbles
      { name: 'Workload', value: 15, color: '#8BC34A', category: 'High Impact', description: 'Heavy workload affecting wellness' },
      { name: 'Recognition', value: 12, color: '#A5D6A7', category: 'Medium Impact', description: 'Insufficient recognition systems' },
      { name: 'Compensation', value: 14, color: '#66BB6A', category: 'High Impact', description: 'Concerns about fair compensation' },
      { name: 'Work-Life', value: 11, color: '#81C784', category: 'Medium Impact', description: 'Work-life balance issues' },
      { name: 'Career Growth', value: 13, color: '#4CAF50', category: 'High Impact', description: 'Limited career advancement' },
      { name: 'Team Culture', value: 10, color: '#C8E6C9', category: 'Medium Impact', description: 'Team dynamic challenges' },
      { name: 'Leadership', value: 12, color: '#7CB342', category: 'High Impact', description: 'Leadership effectiveness' },
      
      // Medium bubbles
      { name: 'Communication', value: 8, color: '#AED581', category: 'Medium Impact', description: 'Communication clarity' },
      { name: 'Resources', value: 7, color: '#DCEDC8', category: 'Low Impact', description: 'Access to resources' },
      { name: 'Training', value: 6, color: '#F1F8E9', category: 'Low Impact', description: 'Professional development' },
      
      // Small bubbles
      { name: 'Tech', value: 4, color: '#E8F5E9', category: 'Low Impact', description: 'Technology issues' },
      { name: 'Cafeteria', value: 3, color: '#E0F2F1', category: 'Low Impact', description: 'Cafeteria options' },
      { name: 'Parking', value: 2, color: '#F1F8E9', category: 'Low Impact', description: 'Parking facilities' },
      { name: 'Office Space', value: 3, color: '#E0F2F1', category: 'Low Impact', description: 'Office environment' },
      { name: 'Benefits', value: 5, color: '#DCEDC8', category: 'Low Impact', description: 'Employee benefits package' }
    ];
    ...
    const nodes = bubbleGroup.selectAll('.node')
      ...
      .append('g')
      .attr('class', 'node')
      .attr('transform', d => `translate(${d.x - packSize/2}, ${d.y - packSize/2})`);
  ...
  return (
    <div className={`w-full ${className}`}>
      ...
      <div className="p-4 sm:p-6">
        ...
        <p className="text-sm text-gray-500 mt-1">Primary concerns reported by employees</p>
