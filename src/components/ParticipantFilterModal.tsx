...
const ParticipantFilterModal: FC<ParticipantFilterModalProps> = ({
  ...
  const [tempJobTitleFilters, setTempJobTitleFilters] = useState<string[]>(currentFilters.jobTitles);
  const [tempDateAddedFilters, setTempDateAddedFilters] = useState<string[]>(currentFilters.dateAdded);
  
  // Get unique job titles and dates from participants
  const uniqueJobTitles = ['All', ...Array.from(new Set(participants.map(p => p.title)))];
  const uniqueDates = ['All', ...Array.from(new Set(participants.map(p => p.dateAdded)))].sort();
            ...
            <div className="space-y-2 max-h-[150px] overflow-y-auto pr-2">
              {uniqueJobTitles.map((title, index) => (
                <div key={index} className="flex items-center">
                  ...
                  />
                  <label htmlFor={`job-title-filter-${index}`} className="ml-2 text-sm text-gray-900">
                    {title}
                  </label>
                </div>
              ))}
            ...
            <div className="space-y-2 max-h-[150px] overflow-y-auto pr-2">
              {uniqueDates.map((date, index) => (
                <div key={index} className="flex items-center">
                  ...
                  />
                  <label htmlFor={`date-added-filter-${index}`} className="ml-2 text-sm text-gray-900">
                    {date}
                  </label>
                </div>
              ))}
